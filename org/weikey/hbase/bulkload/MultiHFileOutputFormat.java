  ...
  throws IOException, InterruptedException {
    return new RecordWriter<ImmutableBytesWritable, KeyValue>() {
      ...
      public void write(ImmutableBytesWritable key, KeyValue value)
          throws IOException, InterruptedException {
    	  String dir = Util.getDir(key.get());
    	  ImmutableBytesWritable inKey = new ImmutableBytesWritable(Bytes.toBytes(dir));
        RecordWriter<ImmutableBytesWritable, KeyValue> writer = writers.get(inKey);
        if (writer == null) {
          ...
          writer = new RecordWriter<ImmutableBytesWritable, KeyValue>() {
           ...
           private HFile.Writer getNewWriter(final HFile.Writer writer,
              ...
              WriterFactory factory = HFile.getWriterFactoryNoCache(conf);
              factory.withPath(fs, StoreFile.getUniqueFile(fs, familydir));
              factory.withBlockSize(blocksize);
              factory.withCompression(compression);
//              RowComparator comparator = new KeyValue.RowComparator(KeyValue.COMPARATOR);
              factory.withComparator(KeyValue.KEY_COMPARATOR);
